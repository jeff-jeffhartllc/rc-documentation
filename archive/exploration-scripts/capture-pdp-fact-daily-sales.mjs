...
const DATASET_ID = '5bdaf9aa-0950-432e-a9ce-eaa7cffb2796';

async function setup() {
  const { execSync } = await import('child_process');
  const USER_DATA = '/tmp/chrome-pdp-fds';
...
async function main() {
  ...
  const asset = '/workspace/assets/domo-regis-factdailysales-pdp-policies.png';
  await page.screenshot({ path: asset, fullPage: true });
  await browser.close();
  fs.writeFileSync('/tmp/pdp-fact-daily-sales.json', JSON.stringify({ ...results, asset }, null, 2));
